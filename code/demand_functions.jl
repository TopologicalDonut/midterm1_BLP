...
export demand_objective_function, shares
...
Φ(δ) = δ + log.(s) - log.(shares(δ,θ₂,X,v,market_id)[1])
...
function shares(δ, θ₂, X, v, market_id)
...
for market in unique(market_id)
    ...
    μ[market_id.==market,:] = X[market_id.==market,:]*(v[market,:,:] .* θ₂')' 
end
